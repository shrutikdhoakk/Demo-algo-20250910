...
# Import available strategies.  We support both v3 (default) and
# the upgraded v7 variant.  Import errors are intentionally silent
# to allow fallback to the original implementation when the new
# module is absent.
...
class BacktestEngine:
    def __init__(
        ...
        # We introspect the dataclass fields to capture default values without creating
        # an instance.  Any YAML keys that overlap with dataclass fields are used to
        # override these defaults.  For backward compatibility we preserve the
        # original behaviour for v3 when the v7 module is not available.
        ...
        # Override defaults with YAML values when present.  Only update keys
        # that exist on the dataclass to avoid accidental injection of
        # unrelated configuration into the strategy.
        ...
        if strategy_name == 'v7' and BreakoutMomentumV7 is not None:
            # Print loaded parameters for v7 for transparency
            print(
            ...
            # Merge legacy overrides for v3: allow breakout_atr_buf, trail_atr_mult, atr_pct_max
